...
ggsave(filename = "./output/dbRDA_stacked_bars_PHI_bowenrealms_sigonly_041118.pdf",bars,width=7,height=7,units="in")
...
diffstats3<-diffstats_phist[lapply(diffstats2, class) =="dist"]
...
medians<-c(apply(extracted_Fst2,MARGIN=2,FUN=median, na.rm=T), Randomized=median(random_diffstats_all_3$value,na.rm=T))

CI95<-cbind(apply(extracted_Fst2,MARGIN=2,FUN=bootstrap_phist),Randomized=bootstrap_phist(random_diffstats_all_3$value))
...
phist_medians<-ggplot(data=medianCI) + geom_pointrange(data=medianCI,mapping=aes(x=barrier,y=medians,ymin=low,ymax=high), inherit.aes=F,fatten=4) + scale_x_discrete(labels=c("Western Indo-Pacific / \n Central Indo-Pacific", "Central Indo-Pacific / \n Eastern Indo-Pacific", "Eastern Indo-Pacific / \n Hawaii", "Eastern Indo-Pacific / \n Marquesas", "Randomized")) + ylab(expression(Phi[ST])) + xlab("Barrier")  + stat_summary(data=extracted_Fst_melt,mapping=aes(x=variable,y=value),fun.data = n_fun, geom = "text", alpha=1, position_fill(vjust=1.2))  + coord_cartesian(ylim=c(0,0.12)) + theme(axis.text.x = element_text(angle=45, hjust=1))
 
medianCI

ggsave(phist_medians,file="./output/fst_medians1.pdf", width=7, height=7, units="in")
