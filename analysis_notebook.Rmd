...
```{r Effect Sizes}
# Look at mean FCT for each hypothesis
FCTs<-NULL
for(h in hypotheses){
...
for(h in names(amovastats)){
  effect<-merge(effect,t(amovastats[[h]]["level2_SSD"]/amovastats[[h]]["total_SSD"]),all=T,sort=F)
}
...
diffstats3<-diffstats_fst[lapply(diffstats_phist, class) =="dist"]
