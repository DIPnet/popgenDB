...
rp<-rp+geom_tile()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  ylim(rev(sort(levels(relprob$Species))))+
  scale_fill_gradient2(low = "blue", mid = "white",
                       high = "red", midpoint = 0.5, space = "rgb",
                       na.value = "grey50", guide = "colourbar")
rp
...
#  if(length(rownames(gslFSTm))<5){nostats<-c(nostats,paste(gsl,"pops<5"));cat("Fewer than 5 sampled populations");next}
  ...
  #       nostats<-c(nostats,paste(gsl,"No Solution"))
  #       
...
if(!is.null(fullgdm)){
  ...
  ispline2<-splinegrab(ispline)
    ...
    #plot both the ispline (red) and the linear model (blue)
ibdplot<- ggplot(data=ibd,mapping=aes(x=distance,y=phist)) + geom_point() +
      geom_smooth(method=lm) +
      geom_line(data=ispline2,mapping=aes(x=V1,y=V2), color="red") +
      ggtitle(label=gsl)
...
if(is.null(fullgdm)){
  ...
  ibdplot<- ggplot(data=ibd,mapping=aes(x=distance,y=phist)) +geom_point()+
    geom_smooth(method=lm) + ggtitle(gsl)
  plotlist[[gsl]]<-ibdplot
}
...
# code to fix Naso Hexacanthus
stats.full[which(stats.full$Species_Locus=="Naso_hexacanthus_CR"),"dist_deviance_explained"]<-1
stats.full[which(stats.full$Species_Locus=="Naso_hexacanthus_CR"),"unexplained_deviance"]<-0
