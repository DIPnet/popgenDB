...
CI95<-cbind(apply(extracted_Fst2,MARGIN=2,FUN=bootstrap_phist),Simulated=bootstrap_phist(simPHISTdf$value),Randomized=bootstrap_phist(random_diffstats_all$value))
...
phist_medians<-ggplot(data=medianCI) + geom_pointrange(data=medianCI,mapping=aes(x=barrier,y=medians,ymin=low,ymax=high), inherit.aes=F,fatten=4) + scale_x_discrete(labels=c("Western Indian/ \n Western Indo-Pacific","Western Indo-Pacific / \n Central Indo-Pacific", "Central Indo-Pacific / \n Eastern Indo-Pacific", "Eastern Indo-Pacific / \n Hawaii", "Eastern Indo-Pacific / \n Marquesas","Simulated", "Randomized")) + ylab(expression(Phi[ST])) + xlab("Barrier")  + stat_summary(data=extracted_Fst_melt,mapping=aes(x=variable,y=value),fun.data = n_fun, geom = "text", alpha=1, position_fill(vjust=1.1))  + coord_cartesian(ylim=c(0,0.105)) + theme(axis.text.x = element_text(angle=45, hjust=1))
