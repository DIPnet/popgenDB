...
ggsave("./output/ibdplots.pdf",ibdplots)
...
write.csv(term.vars,file="./output/dbRDA_term_vars_PHIST_030818.csv")
write.csv(term.pvals,file="./output/dbRDA_term_pvals_PHIST_030818.csv")

 save(all.gsl.rda,file="./output/multibarrier_dbRDA_PHIST_030818.Rdata")
  
write.csv(stats, "./output/multibarrier_dbRDA_PHIST_030818.csv")
...
# first get rid of non-Indo-Pacific samples

dim(ipdb)

ipdb2<-ipdb[ipdb$REALM %in% c("Central Indo-Pacific","Eastern Indo-Pacific","Western Indo-Pacific"),]


dim(ipdb)[1]-dim(ipdb2)[1]
# Removed about 3600 samples

ipdb2$REALM[which(ipdb2$Bowen=="Hawaiian")]<-"Hawaii"
ipdb2$REALM[which(ipdb2$Bowen=="Marquesas")]<-"Marquesas"
ipdb2$REALM[which(ipdb2$Bowen=="Easter")]<-"Easter"

unique(ipdb2$REALM)

# Remove dups
ipdb2<-ipdb2[!ipdb2$Genus_species_locus %in% c(dups,"Ctenochaetus_marginatus_A68"),]
...
```{r}
#diffstats2<-pairwise.structure.mtDNA.db(ipdb=ipdb2, gdist = "PhiST", minseqs = 5, minsamps = 2, mintotalseqs = 0, nrep = 1000, num.cores = 2, ABGD = F, regionalization = "REALM")
#save(diffstats2,"./output/PHIst_for_map.R")
...
diffstats3<-diffstats2[lapply(diffstats2, class) =="dist"]
...
```{r}
# ipdb3<-ipdb2[which(ipdb2$Genus_species_locus %in% names(diffstats3)),]
# 
...
#   }
# 
# random_diffstats<-pairwise.structure.mtDNA.db(ipdb=ipdb3, gdist = "PhiST", minseqs = 5, minsamps = 2, mintotalseqs = 0, nrep = 0, num.cores = 2, ABGD = F, regionalization = "randomregion")
# 
...
# }
# 
# random_diffstats_all[which(random_diffstats_all < 0)]<-0
# 
# random_diffstats_all<-data.frame(variable=factor("Randomized"), value = as.numeric(random_diffstats_all))

#save(random_diffstats_all,file="Randomized_PHIst.R")
...
phist_violins<-ggplot(data=extracted_Fst_melt, aes(x=variable,y=value, alpha=variable)) + geom_violin(alpha=1) + stat_summary(fun.data = n_fun, geom = "text", alpha=1) + geom_jitter(size=0.0) + ylab(expression(Phi[ST])) + xlab("Barrier") + scale_alpha_manual(values=c(1,1,1,1,0), guide=F) + scale_x_discrete(labels=c("Western Indo-Pacific / \n Central Indo-Pacific", "Central Indo-Pacific / \n Eastern Indo-Pacific", "Eastern Indo-Pacific / \n Hawaii", "Eastern Indo-Pacific / \n Marquesas", "Randomized"))
...
medianCI["barrier"]<-rownames(medianCI)

phist_violins2<-phist_violins  + geom_pointrange(data=medianCI,mapping=aes(x=barrier,y=medians,ymin=low,ymax=high), inherit.aes=F,fatten=0.1) + coord_cartesian(ylim=c(0,0.6))

medianCI

#Figure out why N. albicilla is so low!
