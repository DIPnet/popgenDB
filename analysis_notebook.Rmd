...
#filter for species that test at least 4 of the 8 hypotheses
crit_df2<-crit_df[,which(apply(is.na(t(crit_df)),MARGIN=1,FUN=sum) < 5)]

#rank the hypotheses for each species
crit_rank<-as.data.frame(sapply(crit_df2,rank,na.last="keep",ties.method="average"))
...
load(file=file.path(amova_ts_path,"PHIST_ts_rawN_amova_output.Rdata"))
#remove duplicate gsls
amovastats<-lapply(amovastats,remove.dup.gsls)
...
stats.full<-data.frame(Species_Locus=character(0),FullDeviance=numeric(0),FullExplainedDeviance=numeric(0),DistDeviance=numeric(0),FullExplainedDeviance=numeric(0),DeltaDeviance=numeric(0),Pvalue_regions=numeric(0),Pvalue_dist=numeric(0),region_deviance_explained=numeric(0), dist_deviance_explained=numeric(0), both_deviance_explained=numeric(0),unexplained_deviance=numeric(0), stringsAsFactors = F)
...
for(gsl in esu_loci){ #gsl<-"Linckia_laevigata_CO1" "Tridacna_crocea_CO1" "Lutjanus_kasmira_CYB" "Acanthaster_planci_CO1"
  ...
  if(is.null(distgdm)){distgdm<-list(gdmdeviance=fullgdm$nulldeviance,explained=0)}
  if(is.null(regiongdm)){regiongdm<-list(gdmdeviance=fullgdm$nulldeviance,explained=0)}
  #calculate the relative contributions to deviance explained for geographical distance and regions and both
...
B<-ABC-A-C
  ...
  #if no solution on full model, then keep the names of the regions for this species in a matrix with zero values
  if(is.null(fullgdm)){
  ...
  ##############################################################################
    ...
    stats.1<-c(gsl,gdm.full.deviance,gdm.full.explained,gdm.dist.deviance,gdm.dist.explained,deltadev.regions,pvalue_regions.full,pvalue_dist.full, A, C, B, D)
...
for(gsl in esu_loci){ #gsl<-"Linckia_laevigata_CO1" "Tridacna_crocea_CO1" "Lutjanus_kasmira_CYB"
  ...
  sp$sample<-paste(sp$locality,round(sp$decimalLatitude, digits=0),round(sp$decimalLongitude, digits=0),sep="_")  #sets up a variable that matches the name in Fst table
  sp<-sp[order(sp$sample),]
  # Not all localities are included in REALM 
  ...
  regions<-with(locs, data.frame(model.matrix(~REALM+0)))   #one of the predictors is superfluous but will get knocked out during RDA
...
write.csv(term.vars,file="./output/dbRDA_term_vars_PHIST.csv")
write.csv(term.pvals,file="./output/dbRDA_term_pvals_PHIST.csv")

 save(all.gsl.rda,file="./output/multibarrier_dbRDA_PHIST.Rdata")
  
write.csv(stats, "./output/multibarrier_dbRDA_PHIST.csv")
...
# Cull species to match what is in AMOVA
# Make stacked bars that give importance due to distance, due to regionalization and residual.
# Stack significant distance splines on top of each other.
